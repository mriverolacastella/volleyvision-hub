...
def page_export(data):
    section("Exportar")
    modules = st.multiselect("Selecciona módulos", ["Match Report","Data Quality","Post Match","Ataque","Saque","Recepción","Bloqueo","Distribución","Money Time","Objetivos","Acciones crudas"], default=["Match Report","Data Quality","Post Match","Ataque","Saque","Recepción","Bloqueo"])
    def make_excel():
        out = io.BytesIO()
        with pd.ExcelWriter(out, engine="openpyxl") as writer:
            ...
            if "Money Time" in modules:
                mt = apply_moment_filter(data["plays"], "Money Time 18+")
                skill_summary(mt, "A", ["Equipo","Jugador","origen","destino"]).to_excel(writer, sheet_name="Money Time", index=False)
...
def dashboard():
    ...
    tabs = st.tabs(["General", "Data Quality", "Post Match", "Ataque", "Saque", "Recepción", "Bloqueo", "Colocador / Setter Lab", "Money Time", "Por Set", "Match Plan", "Player Sheet", "Vídeo / Clips", "Objetivos", "Exportar"])
    ...
    with tabs[8]: page_money_time(data)
    with tabs[9]: page_by_set(data)
    with tabs[10]: page_match_plan(data)
    with tabs[11]: page_player_sheet(data)
    with tabs[12]: page_video_clips(data)
    with tabs[13]: page_objectives(data)
    with tabs[14]: page_export(data)
