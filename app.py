...
def page_export(data):
    section("Exportar")
    modules = st.multiselect("Selecciona módulos", ["Match Report","Data Quality","Post Match","Ataque","Saque","Recepción","Bloqueo","Distribución","Objetivos","Acciones crudas"], default=["Match Report","Data Quality","Post Match","Ataque","Saque","Recepción","Bloqueo"])
    def make_excel():
        out = io.BytesIO()
...
def dashboard():
    ...
    tabs = st.tabs(["General", "Data Quality", "Post Match", "Ataque", "Saque", "Recepción", "Bloqueo", "Colocador / Setter Lab", "Por Set", "Match Plan", "Player Sheet", "Vídeo / Clips", "Objetivos", "Exportar"])
    with tabs[0]: page_general(data)
    with tabs[1]: page_data_quality(data)
    with tabs[2]: page_post_match(data)
    with tabs[3]: page_attack(data)
    with tabs[4]: page_serve(data)
    with tabs[5]: page_reception(data)
    with tabs[6]: page_block(data)
    with tabs[7]: page_setter(data)
    with tabs[8]: page_by_set(data)
    with tabs[9]: page_match_plan(data)
    with tabs[10]: page_player_sheet(data)
    with tabs[11]: page_video_clips(data)
    with tabs[12]: page_objectives(data)
    with tabs[13]: page_export(data)
