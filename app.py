...
.nav-shell{border:1px solid var(--line);background:linear-gradient(180deg,rgba(20,34,53,.88),rgba(9,18,30,.88));border-radius:22px;padding:.75rem 1rem;margin:.4rem 0 1.1rem;box-shadow:0 16px 40px rgba(0,0,0,.18)}
.nav-title{font-weight:900;color:#f4f7fb;margin-bottom:.35rem;letter-spacing:-.02em}.nav-sub{color:#9aa7b7;font-size:.86rem;margin-bottom:.55rem}
div[role="radiogroup"]{gap:.4rem!important;flex-wrap:wrap!important}div[role="radiogroup"] label{background:#101a28;border:1px solid #2a3b50;border-radius:999px;padding:.45rem .85rem;margin:.15rem;color:#dbe7f3!important;font-weight:800}div[role="radiogroup"] label:has(input:checked){background:linear-gradient(135deg,#f59e0b,#f97316);color:#071018!important;border-color:#f59e0b}
.decision-card{border:1px solid #30445c;background:linear-gradient(135deg,rgba(245,158,11,.12),rgba(34,211,238,.05));border-radius:18px;padding:1rem;margin:.65rem 0}.decision-card b{color:#f59e0b}.decision-card .tag{display:inline-block;border:1px solid #3e526b;border-radius:999px;padding:.15rem .55rem;margin-right:.35rem;color:#cbd5e1;font-size:.78rem;font-weight:800}
</style>
"""
...
def filters(data, skill=None, include_player=False, player_label="Jugador"):
    ...
    with c5: moment = st.selectbox("Tramo del set", list(MONEY_SEGMENTS.keys()) if "MONEY_SEGMENTS" in globals() else ["Todo"], key=f"moment_{skill}_{player_label}")
    player = "Todos"
    if include_player:
        base = filter_plays(plays, team, phase, rot, desde, skill=skill)
        base = apply_moment_filter(base, moment) if "apply_moment_filter" in globals() else base
    ...
    df = apply_moment_filter(df, moment) if "apply_moment_filter" in globals() else df
    return df, team, phase, rot, desde, player, moment
...
def apply_moment_filter(df: pd.DataFrame, segment: str) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    bounds = MONEY_SEGMENTS.get(segment)
    if not bounds:
        return df
    lo, hi = bounds
    if "team_score" not in df:
        return df
    return df[(pd.to_numeric(df["team_score"], errors="coerce").fillna(0) >= lo) & (pd.to_numeric(df["team_score"], errors="coerce").fillna(0) <= hi)]

def data_quality_rows(data: dict) -> pd.DataFrame:
    plays = data.get("plays", pd.DataFrame())
    total = max(len(plays), 1)
    checks = []
    def add(area, ok, detail, score):
        checks.append({"Área": area, "Estado": "OK" if ok else "Revisar", "Detalle": detail, "Fiabilidad %": score})
    add("Acciones", not plays.empty, f"{len(plays)} acciones detectadas", 100 if not plays.empty else 0)
    sets = data.get("sets", [])
    add("Sets", bool(sets), f"{len(sets)} sets detectados", min(100, len(sets)*20) if sets else 0)
    lineups = data.get("lineups", {})
    add("Formaciones", bool(lineups), f"{len(lineups)} sets con lineup", min(100, len(lineups)*25) if lineups else 0)
    if not plays.empty:
        add("Zonas de origen", (plays["origen"].ne("Sin zona")).mean() > .25, f"{(plays['origen'].ne('Sin zona')).mean()*100:.1f}% acciones con zona origen", round((plays["origen"].ne("Sin zona")).mean()*100,1))
        add("Zonas de destino", (plays["destino"].ne("Sin zona")).mean() > .25, f"{(plays['destino'].ne('Sin zona')).mean()*100:.1f}% acciones con zona destino", round((plays["destino"].ne("Sin zona")).mean()*100,1))
        add("Rotaciones", (plays["rotation"].astype(int) > 0).mean() > .25, f"{(plays['rotation'].astype(int)>0).mean()*100:.1f}% acciones con rotación", round((plays["rotation"].astype(int)>0).mean()*100,1))
        add("K1/K2", plays["phase"].isin(["K1","K2"]).mean() > .8, f"{plays['phase'].isin(['K1','K2']).mean()*100:.1f}% acciones clasificadas", round(plays["phase"].isin(["K1","K2"]).mean()*100,1))
        add("Colocador", ("setter" in plays and plays.get("setter", pd.Series(dtype=str)).astype(str).ne("").mean() > .05), f"{plays.get('setter', pd.Series(['']*len(plays))).astype(str).ne('').mean()*100:.1f}% ataques con colocador asociado", round(plays.get("setter", pd.Series(['']*len(plays))).astype(str).ne("").mean()*100,1))
        add("Direcciones", ((plays["origen"].ne("Sin zona")) & (plays["destino"].ne("Sin zona"))).mean() > .15, f"{((plays['origen'].ne('Sin zona')) & (plays['destino'].ne('Sin zona'))).mean()*100:.1f}% acciones con origen+destino", round(((plays["origen"].ne("Sin zona")) & (plays["destino"].ne("Sin zona"))).mean()*100,1))
    return pd.DataFrame(checks)

def quality_score(data: dict) -> float:
    q = data_quality_rows(data)
    return round(pd.to_numeric(q["Fiabilidad %"], errors="coerce").fillna(0).mean(), 1) if not q.empty else 0

def target_status(value: float, target: float, higher_better=True, tolerance=0.9):
    if higher_better:
        if value >= target: return "🟢", "OK"
        if value >= target * tolerance: return "🟡", "Cerca"
        return "🔴", "Bajo"
    else:
        if value <= target: return "🟢", "OK"
        if value <= target / tolerance: return "🟡", "Cerca"
...
def team_core_metrics(data: dict, team: str) -> dict:
    plays = data.get("plays", pd.DataFrame())
    d = plays[plays["Equipo"] == team] if not plays.empty else plays
    sets_n = max(1, len(data.get("sets", [])) or int(d["set"].nunique()) if not d.empty else 1)
    at = d[d.skill_code=="A"] if not d.empty else d
    sq = d[d.skill_code=="S"] if not d.empty else d
    rec = d[d.skill_code=="R"] if not d.empty else d
    blq = d[d.skill_code=="B"] if not d.empty else d
    errors = int((d.eval_code=="=").sum()) if not d.empty else 0
    pts = int(((at.eval_code=="#").sum() if not at.empty else 0) + ((sq.eval_code=="#").sum() if not sq.empty else 0) + ((blq.eval_code=="#").sum() if not blq.empty else 0))
    return {
        "Ataque Eff%": pct((at.eval_code=="#").sum()-(at.eval_code=="=").sum(), len(at)) if len(at) else 0,
        "Recepción #+!%": pct((rec.eval_code.isin(["#","+","!"])).sum(), len(rec)) if len(rec) else 0,
        "Saque Value": int((sq.eval_code=="#").sum()*3 + (sq.eval_code.isin(["+","!"])).sum() - (sq.eval_code=="=").sum()) if len(sq) else 0,
        "Bloqueos/set": round((blq.eval_code=="#").sum()/sets_n, 2) if len(blq) else 0,
        "Errores/set": round(errors/sets_n, 2),
        "Puntos/set": round(pts/sets_n, 2),
    }

def operational_targets_table(data: dict, team: str, targets: dict) -> pd.DataFrame:
    m = team_core_metrics(data, team)
    rows = []
    for key, target in targets.items():
        higher = key != "Errores/set"
        value = float(m.get(key, 0))
        icon, state = target_status(value, target, higher_better=higher)
        rows.append({"Objetivo": key, "Actual": value, "Meta": target, "Estado": icon, "Lectura": state})
    return pd.DataFrame(rows)

def serve_value_by_player(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame()
    s = df[df["skill_code"]=="S"].copy()
    if s.empty:
        return pd.DataFrame()
    base = skill_summary(s, "S", ["Equipo","Jugador"])
    if base.empty:
        return base
    base["Serve Value"] = base["#"]*3 + base["+"] + base["!"] - base["="]
    return base.sort_values("Serve Value", ascending=False)

def phase_breakdown(data: dict, team: str) -> pd.DataFrame:
    plays = data.get("plays", pd.DataFrame())
    d = plays[plays["Equipo"] == team] if not plays.empty else plays
    rows = []
    for ph in ["K1","K2"]:
        sub = d[d["phase"]==ph] if not d.empty else d
        at = sub[sub.skill_code=="A"] if not sub.empty else sub
        sq = sub[sub.skill_code=="S"] if not sub.empty else sub
        rec = sub[sub.skill_code=="R"] if not sub.empty else sub
        rows.append({
            "Fase": ph,
            "Acciones": len(sub),
            "AT Eff%": pct((at.eval_code=="#").sum()-(at.eval_code=="=").sum(), len(at)) if len(at) else 0,
            "REC #+!%": pct(rec.eval_code.isin(["#","+","!"]).sum(), len(rec)) if len(rec) else 0,
            "SQ Value": int((sq.eval_code=="#").sum()*3 + (sq.eval_code.isin(["+","!"])).sum() - (sq.eval_code=="=").sum()) if len(sq) else 0,
            "Errores": int((sub.eval_code=="=").sum()) if len(sub) else 0
        })
    return pd.DataFrame(rows)

def role_targets_defaults():
    return {"Ataque Eff%": 40.0, "Recepción #+!%": 60.0, "Saque Value": 5.0, "Bloqueos/set": 2.0, "Errores/set": 6.0, "Puntos/set": 17.0}

def trend_by_match(data: dict, team: str, metric: str) -> pd.DataFrame:
    plays = data.get("plays", pd.DataFrame())
    if plays.empty or "source_match" not in plays.columns:
        return pd.DataFrame()
    rows = []
    for sm, g in plays[plays["Equipo"]==team].groupby("source_match"):
        fake = {**data, "plays": g, "sets": [{"set": s} for s in sorted(g["set"].unique())]}
        val = team_core_metrics(fake, team).get(metric, 0)
        rows.append({"Partido": sm, metric: val})
    return pd.DataFrame(rows)



def rotation_report(data: dict, team: str) -> pd.DataFrame:
    plays = data.get("plays", pd.DataFrame())
    if plays.empty:
        return pd.DataFrame()
    d = plays[(plays["Equipo"] == team) & (plays["rotation_label"].astype(str) != "")].copy()
    rows = []
    for rot, g in d.groupby("rotation_label"):
        at = g[g.skill_code=="A"]; rec = g[g.skill_code=="R"]; sq = g[g.skill_code=="S"]; blq = g[g.skill_code=="B"]
        rows.append({
            "Rotación": rot,
            "Acciones": len(g),
            "AT Eff%": pct((at.eval_code=="#").sum()-(at.eval_code=="=").sum(), len(at)) if len(at) else 0,
            "REC #+!%": pct(rec.eval_code.isin(["#","+","!"]).sum(), len(rec)) if len(rec) else 0,
            "SQ Value": int((sq.eval_code=="#").sum()*3 + sq.eval_code.isin(["+","!"]).sum() - (sq.eval_code=="=").sum()) if len(sq) else 0,
            "BLQ #": int((blq.eval_code=="#").sum()) if len(blq) else 0,
            "Errores =": int((g.eval_code=="=").sum()),
            "Puntos #": int((g.eval_code=="#").sum()),
        })
    out = pd.DataFrame(rows)
    if not out.empty:
        out["Balance"] = out["Puntos #"] - out["Errores ="]
    return out.sort_values("Rotación")

def player_production_report(data: dict, team: str) -> pd.DataFrame:
    rep = player_report(data)
    if rep.empty:
        return pd.DataFrame()
    sets_n = max(1, len(data.get("sets", [])) or data.get("plays", pd.DataFrame()).get("set", pd.Series(dtype=int)).nunique())
    df = rep[rep["Equipo"] == team].copy()
    if df.empty:
        return df
    df["PTS/set"] = (pd.to_numeric(df["PTS"], errors="coerce").fillna(0) / sets_n).round(2)
    errs = pd.to_numeric(df.get("SQ =",0), errors="coerce").fillna(0) + pd.to_numeric(df.get("REC =",0), errors="coerce").fillna(0) + pd.to_numeric(df.get("AT =",0), errors="coerce").fillna(0) + pd.to_numeric(df.get("BLQ =",0), errors="coerce").fillna(0)
    df["=/set"] = (errs / sets_n).round(2)
    df["Aportación"] = pd.to_numeric(df["PTS"], errors="coerce").fillna(0) - errs
    cols = ["#","Jugador","Posición","PTS","PTS/set","=/set","Aportación","AT Eff%","REC #+!%","SQ #","SQ =","BLQ #"]
    return df[[c for c in cols if c in df.columns]].sort_values(["PTS/set","Aportación"], ascending=False)

def coaching_decisions(data: dict, team: str) -> list[tuple[str,str,str]]:
    plays = data.get("plays", pd.DataFrame())
    if plays.empty:
        return []
    d = plays[plays["Equipo"] == team]
    decisions = []
    rot = rotation_report(data, team)
    if not rot.empty:
        weak = rot.sort_values(["Balance","AT Eff%"], ascending=[True, True]).iloc[0]
        strong = rot.sort_values(["Balance","SQ Value"], ascending=[False, False]).iloc[0]
        decisions.append(("Rotación a revisar", f"{weak['Rotación']} necesita trabajo específico", f"Balance {weak['Balance']} · AT Eff% {weak['AT Eff%']} · errores {weak['Errores =']}"))
        decisions.append(("Rotación fuerte", f"{strong['Rotación']} puede ser punto de partida del plan", f"Balance {strong['Balance']} · SQ Value {strong['SQ Value']}"))
    sv = serve_value_by_player(d)
    if not sv.empty:
        top = sv.sort_values("Serve Value", ascending=False).iloc[0]
        low = sv.sort_values("Serve Value", ascending=True).iloc[0]
        decisions.append(("Saque", f"Potenciar el turno de {top['Jugador']}", f"Serve Value {top['Serve Value']}. Revisar riesgo con {low['Jugador']} si su valor sigue bajo."))
    prod = player_production_report(data, team)
    if not prod.empty:
        top = prod.iloc[0]
        decisions.append(("Producción", f"Referencia ofensiva: {top['Jugador']}", f"{top.get('PTS/set','-')} puntos/set · aportación {top.get('Aportación','-')}"))
    return decisions
...
def page_attack(data):
    section("Ataque")
    df, team, phase, rot, desde, player, moment = filters(data, "A", True, "Atacante")
    cols = st.columns(5)
...
def page_serve(data):
    section("Saque")
    df, team, phase, rot, desde, player, moment = filters(data, "S", True, "Sacador")
    cols = st.columns(5)
...
def page_reception(data):
    section("Recepción")
    plays = data["plays"]
    df, team, phase, rot, desde, player, moment = filters(data, "R", True, "Receptor")
...
def page_block(data):
    section("Bloqueo")
    df, team, phase, rot, desde, player, moment = filters(data, "B", True, "Bloqueador")
    cols = st.columns(5)
...
def page_setter(data):
    ...
    plays = data["plays"]
    df, team, phase, rot, desde, setter, moment = filters(data, "E", True, "Colocador")
    ...
    att = apply_moment_filter(att, moment)
...
def build_match_plan(data, key_prefix="mp"):
    plays = data["plays"]
    teams = team_names(data)
    c1,c2 = st.columns(2)
    with c1: my = st.selectbox("Mi equipo", teams, key=f"{key_prefix}_my")
    with c2: rival = st.selectbox("Rival", [t for t in teams if t != my], key=f"{key_prefix}_rival")
...
def page_match_plan(data):
    ...
    plan = build_match_plan(data, key_prefix="mp")
...
def page_rotations(data):
    section("Rotaciones / Sideout-Breakpoint")
    teams = team_names(data)
    team = st.selectbox("Equipo", teams, key="rot_team")
    st.markdown("<div class='mini-help'>Lectura rápida por rotación: combina ataque, recepción, saque, bloqueo y balance para detectar rotaciones fuertes y rotaciones a entrenar.</div>", unsafe_allow_html=True)
    rot = rotation_report(data, team)
    if rot.empty:
        st.info("No hay rotaciones suficientes en este archivo.")
        return
    weak = rot.sort_values(["Balance","AT Eff%"], ascending=[True, True]).iloc[0]
    strong = rot.sort_values(["Balance","SQ Value"], ascending=[False, False]).iloc[0]
    c1,c2,c3 = st.columns(3)
    with c1: card("Rotación fuerte", strong["Rotación"], f"Balance {strong['Balance']} · SQ Value {strong['SQ Value']}", "ok")
    with c2: card("Rotación a revisar", weak["Rotación"], f"Balance {weak['Balance']} · AT Eff% {weak['AT Eff%']}", "bad")
    with c3: card("Rotaciones detectadas", len(rot), "P1-P6 si el .dvw lo permite", "accent")
    pro_table(rot, 420, "Informe por rotación")
    st.markdown("### Decisiones sugeridas")
    for title, action, evidence in coaching_decisions(data, team):
        st.markdown(f"<div class='decision-card'><span class='tag'>{title}</span><b>{action}</b><br><span class='muted'>{evidence}</span></div>", unsafe_allow_html=True)

def page_production(data):
    section("Producción / Tendencia")
    teams = team_names(data)
    team = st.selectbox("Equipo", teams, key="prod_team")
    st.markdown("<div class='mini-help'>Inspirado en el vídeo: no mirar solo puntos totales, sino puntos por set, errores por set, aportación y evolución acumulada cuando cargas varios partidos.</div>", unsafe_allow_html=True)
    prod = player_production_report(data, team)
    pro_table(prod, 480, "Producción por jugador")
    st.markdown("### Serve Value")
    pro_table(serve_value_by_player(data["plays"][data["plays"]["Equipo"]==team]), 360, "Valor de saque = (# × 3) + positivos - errores")
    if "source_match" in data.get("plays", pd.DataFrame()).columns:
        st.markdown("### Evolución acumulada")
        metric = st.selectbox("Métrica", ["Puntos/set","Ataque Eff%","Recepción #+!%","Saque Value","Errores/set"], key="prod_metric")
        if metric == "Puntos/set": metric_core = "Puntos/set"
        elif metric == "Ataque Eff%": metric_core = "Ataque Eff%"
        elif metric == "Recepción #+!%": metric_core = "Recepción #+!%"
        elif metric == "Saque Value": metric_core = "Saque Value"
        else: metric_core = "Errores/set"
        tr = trend_by_match(data, team, metric_core)
        if not tr.empty:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=list(range(1,len(tr)+1)), y=tr[metric_core], mode="lines+markers", name=metric_core))
            fig.update_layout(height=360, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#f4f7fb"), xaxis=dict(title="Partido", gridcolor="#233244"), yaxis=dict(title=metric_core, gridcolor="#233244"))
            st.plotly_chart(fig, use_container_width=True)
            pro_table(tr, 280)
    else:
        st.info("La evolución aparece cuando usas la vista acumulada por equipo.")

def page_decision_center(data):
    section("Centro de decisiones")
    teams = team_names(data)
    team = st.selectbox("Equipo", teams, key="dec_team")
    st.markdown("<div class='mini-help'>Resumen de lectura técnica: dato detectado → interpretación → decisión. Pensado para postpartido y preparación del siguiente entrenamiento.</div>", unsafe_allow_html=True)
    decisions = coaching_decisions(data, team)
    if not decisions:
        st.info("No hay datos suficientes para generar decisiones.")
        return
    for title, action, evidence in decisions:
        st.markdown(f"<div class='decision-card'><span class='tag'>{title}</span><b>{action}</b><br><span class='muted'>{evidence}</span></div>", unsafe_allow_html=True)
    st.markdown("### Prioridades de entrenamiento")
    targets = operational_targets_table(data, team, role_targets_defaults())
    focus = targets[targets["Estado"].astype(str).str.contains("🔴|🟡", regex=True)]
    if focus.empty:
        st.success("No aparecen alertas fuertes en los objetivos principales. Mantener trabajo y revisar detalles por rotación.")
    else:
        pro_table(focus, 280)
def page_export(data):
    section("Exportar")
    modules = st.multiselect("Selecciona módulos", ["Match Report","Data Quality","Post Match","Rotaciones","Producción","Ataque","Saque","Recepción","Bloqueo","Distribución","Objetivos","Acciones crudas"], default=["Match Report","Data Quality","Post Match","Rotaciones","Producción","Ataque","Saque","Recepción","Bloqueo"])
    def make_excel():
        out = io.BytesIO()
        with pd.ExcelWriter(out, engine="openpyxl") as writer:
            if "Match Report" in modules: player_report(data).to_excel(writer, sheet_name="Match Report", index=False)
            if "Data Quality" in modules: data_quality_rows(data).to_excel(writer, sheet_name="Data Quality", index=False)
            if "Post Match" in modules:
                rows=[]
                for t in team_names(data):
                    m=team_core_metrics(data,t); m["Equipo"]=t; rows.append(m)
                pd.DataFrame(rows).to_excel(writer, sheet_name="Post Match", index=False)
            if "Objetivos" in modules:
                rows=[]
                for t in team_names(data):
                    tab=operational_targets_table(data,t,role_targets_defaults()); tab.insert(0,"Equipo",t); rows.append(tab)
                pd.concat(rows, ignore_index=True).to_excel(writer, sheet_name="Objetivos", index=False) if rows else pd.DataFrame().to_excel(writer, sheet_name="Objetivos", index=False)
            if "Rotaciones" in modules:
                rows=[]
                for t in team_names(data):
                    tab=rotation_report(data,t); tab.insert(0,"Equipo",t); rows.append(tab)
                pd.concat(rows, ignore_index=True).to_excel(writer, sheet_name="Rotaciones", index=False) if rows else pd.DataFrame().to_excel(writer, sheet_name="Rotaciones", index=False)
            if "Producción" in modules:
                rows=[]
                for t in team_names(data):
                    tab=player_production_report(data,t); tab.insert(0,"Equipo",t); rows.append(tab)
                pd.concat(rows, ignore_index=True).to_excel(writer, sheet_name="Produccion", index=False) if rows else pd.DataFrame().to_excel(writer, sheet_name="Produccion", index=False)
...
# ----------------------------- PRO PAGES -----------------------------
def page_data_quality(data):
    section("Data Quality")
    q = data_quality_rows(data)
    score = quality_score(data)
    c1,c2,c3 = st.columns(3)
    with c1: card("Calidad global", f"{score}%", "Fiabilidad media del archivo", "ok" if score >= 75 else "accent")
    with c2: card("Acciones", len(data.get("plays", pd.DataFrame())), "Acciones leídas del .dvw")
    with c3: card("Partidos", data.get("aggregate_matches", 1), "1 partido o acumulado")
    st.markdown("<div class='mini-help'>Este módulo evita tomar decisiones con datos incompletos. Si un apartado aparece como limitado, la app lo muestra, pero no deberíamos basar un Match Plan fuerte en ese dato.</div>", unsafe_allow_html=True)
    pro_table(q, 420)

def page_objectives(data):
    section("Objetivos / Semáforos")
    teams = team_names(data)
    team = st.selectbox("Equipo", teams, key="obj_team")
    st.markdown("<div class='mini-help'>Define objetivos operativos. El sistema marca en verde, amarillo o rojo si el equipo cumple los estándares. Inspirado en informes postpartido de staff: datos rápidos, visuales y accionables.</div>", unsafe_allow_html=True)
    defaults = role_targets_defaults()
    cols = st.columns(3)
    targets = {}
    for i,(k,v) in enumerate(defaults.items()):
        with cols[i%3]:
            targets[k] = st.number_input(k, value=float(v), step=1.0, key=f"target_{k}")
    table = operational_targets_table(data, team, targets)
    pro_table(table, 300)
    st.markdown("### Tendencia acumulada")
    metric = st.selectbox("Métrica de tendencia", list(defaults.keys()), key="trend_metric")
    tr = trend_by_match(data, team, metric)
    if not tr.empty:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=list(range(1,len(tr)+1)), y=tr[metric], mode="lines+markers", name=metric))
        fig.update_layout(height=360, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#f4f7fb"), xaxis=dict(title="Partido", gridcolor="#233244"), yaxis=dict(title=metric, gridcolor="#233244"))
        st.plotly_chart(fig, use_container_width=True)
        pro_table(tr, 280)
    else:
        st.info("La tendencia aparece cuando trabajas en vista acumulada por equipo.")

def page_post_match(data):
    section("Post Match Report")
    teams = team_names(data)
    team = st.selectbox("Equipo analizado", teams, key="pm_team")
    metrics = team_core_metrics(data, team)
    c = st.columns(6)
    for col,(k,v) in zip(c, metrics.items()):
        with col: card(k, v)
    st.markdown("### Lectura rápida")
    targets = role_targets_defaults()
    obj = operational_targets_table(data, team, targets)
    bad = obj[obj["Estado"].astype(str).str.contains("🔴")]
    warn = obj[obj["Estado"].astype(str).str.contains("🟡")]
    if bad.empty and warn.empty:
        st.success("El equipo cumple o se acerca a los objetivos principales. Mantener plan de entrenamiento y reforzar detalles por rotación.")
    else:
        for _, r in pd.concat([bad, warn]).iterrows():
            st.markdown(f"<div class='insight'><b>{r['Objetivo']}:</b> {r['Estado']} {r['Lectura']} · actual {r['Actual']} vs meta {r['Meta']}<br><span class='muted'>Prioridad de entrenamiento: revisar vídeo y acciones asociadas a este fundamento.</span></div>", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    with c1:
        st.markdown("### Fases K1/K2")
        pro_table(phase_breakdown(data, team), 300)
    with c2:
        st.markdown("### Serve Value")
        pro_table(serve_value_by_player(data["plays"][data["plays"]["Equipo"]==team]), 300)
    st.markdown("### Acciones por set")
    byset = data["plays"][data["plays"]["Equipo"]==team].groupby(["set","skill","eval_code"]).size().reset_index(name="Total") if not data["plays"].empty else pd.DataFrame()
    pro_table(byset, 360)

def page_money_time(data):
    section("Money Time")
    teams = team_names(data)
    c1,c2 = st.columns(2)
    with c1: team = st.selectbox("Equipo", teams, key="mt_team")
    with c2: segment = st.selectbox("Momento del set", list(MONEY_SEGMENTS.keys()), index=4, key="mt_seg")
    d = data["plays"][data["plays"]["Equipo"]==team]
    d = apply_moment_filter(d, segment)
    st.markdown(f"<div class='mini-help'>Analizando <b>{segment}</b>. Úsalo para detectar si el equipo o rival cambia comportamiento en puntos de presión.</div>", unsafe_allow_html=True)
    c = st.columns(5)
    with c[0]: card("Acciones", len(d))
    with c[1]: card("Errores =", int((d.eval_code=="=").sum()) if not d.empty else 0, cls="bad")
    with c[2]: card("Puntos #", int((d.eval_code=="#").sum()) if not d.empty else 0, cls="ok")
    at = d[d.skill_code=="A"] if not d.empty else d
    rec = d[d.skill_code=="R"] if not d.empty else d
    with c[3]: card("AT Eff%", pct((at.eval_code=="#").sum()-(at.eval_code=="=").sum(), len(at)) if len(at) else 0)
    with c[4]: card("REC #+!%", pct(rec.eval_code.isin(["#","+","!"]).sum(), len(rec)) if len(rec) else 0)
    c1,c2 = st.columns(2)
    with c1:
        st.markdown("### Ataque en este tramo")
        pro_table(skill_summary(d, "A", ["Jugador","origen"]).sort_values("Total", ascending=False), 380)
    with c2:
        st.markdown("### Saque en este tramo")
        pro_table(serve_value_by_player(d), 380)
    st.markdown("### Distribución por rotación")
    pro_table(skill_summary(d, "A", ["rotation_label","origen"]).sort_values(["rotation_label","Total"], ascending=[True,False]), 360)

def page_player_sheet(data):
    section("Player Sheet")
    st.markdown("<div class='mini-help'>Hoja sencilla para jugadoras: pocas consignas, visuales y recordables. Pensada para imprimir o copiar en charla de vídeo.</div>", unsafe_allow_html=True)
    plan = build_match_plan(data, key_prefix="ps")
    st.markdown(f"""
    <div class='print-sheet'>
    <h2>Player Sheet · {plan['my']} vs {plan['rival']}</h2>
    <h3>3 ideas del partido</h3>
    <ol>
      <li><b>Saque:</b> buscar {plan['weak_rec_zone']} / {plan['weak_receptor']}.</li>
      <li><b>Bloqueo:</b> prioridad sobre {plan['top_zone']} y atacante {plan['top_attacker']}.</li>
      <li><b>Rotación:</b> salida recomendada {plan['suggested_rotation']} si el rival confirma {plan['rival_start']}.</li>
    </ol>
    <h3>Keywords</h3>
    <ul>
      <li><b>Presión Z5/Z6:</b> sacar sobre la zona débil detectada.</li>
      <li><b>Leer salida:</b> con recepción negativa rival, preparar {plan['setter_neg_target']}.</li>
      <li><b>Money Time:</b> mantener el plan y no regalar saque desde punto 18.</li>
    </ul>
    <h3>Notas</h3>
    <p style='min-height:90px;border:1px dashed #94a3b8;border-radius:12px;padding:.7rem'> </p>
    </div>
    """, unsafe_allow_html=True)

def page_video_clips(data):
    section("Vídeo / Clips")
    st.markdown("<div class='mini-help'>Módulo preparado para trabajo pre/post partido con vídeo. Puedes cargar el vídeo y filtrar acciones. Si el .dvw trae timestamps fiables, se podrán asociar acciones a cortes. Si no los trae, este módulo queda como buscador de acciones y preparación de clips.</div>", unsafe_allow_html=True)
    video = st.file_uploader("Sube el vídeo del partido (.mp4/.mov)", type=["mp4","mov","m4v"], accept_multiple_files=False, key="video_upload")
    if video:
        st.video(video)
    plays = data.get("plays", pd.DataFrame())
    has_time_cols = [c for c in plays.columns if any(x in c.lower() for x in ["time","video","frame"])]
    if has_time_cols:
        st.success(f"Campos temporales detectados: {', '.join(has_time_cols)}")
    else:
        st.warning("En estos .dvw no se detectan timestamps de vídeo fiables. Se puede usar filtrado de acciones, pero los clips automáticos requieren una sincronización posterior.")
    st.markdown("### Buscador de acciones para clips")
    c1,c2,c3,c4 = st.columns(4)
    with c1: team = st.selectbox("Equipo", ["Todos"]+team_names(data), key="vid_team")
    with c2: skill = st.selectbox("Gesto", ["Todos","S","R","A","B","E","D"], format_func=lambda x: {"S":"Saque","R":"Recepción","A":"Ataque","B":"Bloqueo","E":"Colocación","D":"Defensa"}.get(x,x), key="vid_skill")
    with c3: ev = st.selectbox("Valoración", ["Todos"]+EVALS, key="vid_eval")
    with c4: segment = st.selectbox("Momento", list(MONEY_SEGMENTS.keys()), key="vid_seg")
    df = plays.copy()
    if team != "Todos": df = df[df["Equipo"]==team]
    if skill != "Todos": df = df[df["skill_code"]==skill]
    if ev != "Todos": df = df[df["eval_code"]==ev]
    df = apply_moment_filter(df, segment)
    cols = ["set","team_score","opp_score","Equipo","Jugador","skill","eval_code","origen","destino","rotation_label","phase","raw"]
    pro_table(df[[c for c in cols if c in df.columns]].head(200), 520)
    st.info("Siguiente fase: cuando cerremos lectura de timestamps, añadiremos botones para generar clips con FFmpeg y montajes automáticos por filtros.")
...
def dashboard():
    ...
    st.markdown("<div class='nav-shell'><div class='nav-title'>Menú de análisis</div><div class='nav-sub'>Enfoque prepartido y postpartido. Selecciona un módulo: solo se carga la página elegida para que la app sea más rápida y estable.</div>", unsafe_allow_html=True)
    pages = ["General", "Calidad de datos", "Post Match", "Centro de decisiones", "Rotaciones", "Producción", "Ataque", "Saque", "Recepción", "Bloqueo", "Colocador / Setter Lab", "Match Plan", "Objetivos", "Exportar"]
    page = st.radio("Módulo", pages, horizontal=True, label_visibility="collapsed", key="main_nav")
    st.markdown("</div>", unsafe_allow_html=True)
    if page == "General": page_general(data)
    elif page == "Calidad de datos": page_data_quality(data)
    elif page == "Post Match": page_post_match(data)
    elif page == "Centro de decisiones": page_decision_center(data)
    elif page == "Rotaciones": page_rotations(data)
    elif page == "Producción": page_production(data)
    elif page == "Ataque": page_attack(data)
    elif page == "Saque": page_serve(data)
    elif page == "Recepción": page_reception(data)
    elif page == "Bloqueo": page_block(data)
    elif page == "Colocador / Setter Lab": page_setter(data)
    elif page == "Match Plan": page_match_plan(data)
    elif page == "Objetivos": page_objectives(data)
    elif page == "Exportar": page_export(data)
