...
def _num_series(x):
    """Convierte columnas pandas a números para evitar errores en Streamlit Cloud."""
    return pd.to_numeric(x, errors="coerce").fillna(0)

def _safe_eff(pos, neg, total):
    total_num = _num_series(total).replace(0, 1)
    return ((_num_series(pos) - _num_series(neg)) / total_num * 100).round(1)
...
def render_attack(data, key="att"):
    ...
    with left:
        ...
        if not zg.empty: zg["Eff%"] = _safe_eff(zg["#"], zg["="], zg["total"])
    ...
    with right:
        ...
        if player == "Selecciona atacante":
            ...
            if not dirs.empty: dirs["Eff%"] = _safe_eff(dirs["#"], dirs["="], dirs["total"])
...
def _reception_zone_summary(rec: pd.DataFrame, group_col: str = "origen") -> pd.DataFrame:
    ...
    if not out.empty:
        ...
        out["=%"] = (out["="] / out["Total"].replace(0,1) * 100).round(1)
        out["Eff%"] = _safe_eff(_num_series(out["#"]) + _num_series(out["+"]) + _num_series(out["!"]), out["="], out["Total"])
    return out
...
def render_block(data, key="blk"):
    """Módulo de bloqueo robusto: nunca debe romper la app por tipos numéricos."""
    df, team_label = filtered_context(data, key)
    teams = {data["home_team"]["name"]: "home", data["away_team"]["name"]: "away"}
    ...
    blk_all = df[df["skill_code"] == "B"].copy() if "skill_code" in df.columns else pd.DataFrame()
    rival_att = opp[opp["skill_code"] == "A"].copy() if (not opp.empty and "skill_code" in opp.columns) else pd.DataFrame()

    c1, c2 = st.columns([1, 1])
    blockers = sorted([x for x in blk_all.get("jugador", pd.Series(dtype=str)).dropna().unique() if x]) if not blk_all.empty else []
    with c1:
        blocker = st.selectbox("Bloqueador", ["Todos"] + blockers, key=f"{key}_player")
    with c2:
        st.caption("Acciones de bloqueo B del .dvw cruzadas con el ataque rival para preparar bloqueo-defensa.")

    view = blk_all.copy()
    if blocker != "Todos" and not view.empty:
    ...
    _metric_strip([
        ("BLQ acciones", total), ("BLQ #", pts), ("BLQ =", errs),
        ("Continuidad +!-", cont), ("Balance", balance),
        ("BLQ#/set", per_set), ("Eff%", f"{eff}%"),
    ])
    ...
    zone = pd.DataFrame(columns=["origen", "Total", "#", "=", "Eff%"])
    if not view.empty and "origen" in view.columns:
        ...
        if not zone.empty:
            for col in ["Total", "#", "="]:
                zone[col] = pd.to_numeric(zone[col], errors="coerce").fillna(0)
            zone["Eff%"] = _safe_eff(zone["#"], zone["="], zone["Total"])
    ...
    with right:
        if not blk_all.empty:
            ...
            base["Eff%"] = _safe_eff(base["#"], base["="], base["Acciones"])
            rot_best = players.sort_values(["#", "Acciones"], ascending=[False, False]).drop_duplicates(["jugador", "dorsal"])[["jugador", "dorsal", "rotacion"]].rename(columns={"rotacion": "Rotación más fuerte"}) if not players.empty else pd.DataFrame(columns=["jugador","dorsal","Rotación más fuerte"])
    ...
    with l2:
        if not view.empty:
            ...
            for col in ["Acciones", "#", "="]:
                rot_df[col] = pd.to_numeric(rot_df[col], errors="coerce").fillna(0)
    ...
    with r2:
        if not rival_att.empty:
            ...
            for col in ["Ataques_rival", "Rival #", "Rival =", "BLQ #", "BLQ =", "Acciones BLQ"]:
                scout[col] = pd.to_numeric(scout[col], errors="coerce").fillna(0)
            scout["Ataque rival Eff%"] = _safe_eff(scout["Rival #"], scout["Rival ="], scout["Ataques_rival"])
            max_att = scout["Ataques_rival"].max() if not scout.empty else 0
            scout["Prioridad"] = scout.apply(lambda r: "Alta" if r["Ataques_rival"] >= max_att or r["Ataque rival Eff%"] > 35 else "Media", axis=1)
    ...
    if not rival_att.empty:
        ...
        if not rz.empty:
            for col in ["Total", "Kills", "Errores"]:
                rz[col] = pd.to_numeric(rz[col], errors="coerce").fillna(0)
            rz["Eff%"] = _safe_eff(rz["Kills"], rz["Errores"], rz["Total"])
...
def render_distribution(data, key="dist"):
    ...
    with right:
        ...
        if not attacks.empty:
            ...
            zone_table["Eff%"] = _safe_eff(zone_table["Kills"], zone_table["Errores"], zone_table["Balones"])
    ...
    if not attacks.empty:
        ...
        combo["Eff%"] = _safe_eff(combo["#"], combo["="], combo["Total"])
        ctx=[]
...
def generate_match_plan(data: dict, my_team: str, rival_team: str, fase: str = "Total", desde: int = 0) -> dict:
    ...
    if not att.empty:
        ...
        az["Volumen %"]=(az["Total"]/max(len(att),1)*100).round(1); az["Eff%"]=_safe_eff(az["#"], az["="], az["Total"])
        ...
        byp["Volumen %"]=(byp["Total"]/max(len(att),1)*100).round(1); byp["Eff%"]=_safe_eff(byp["#"], byp["="], byp["Total"])
        ...
        if not dirs.empty:
            dirs["Eff%"] = _safe_eff(dirs["#"], dirs["="], dirs["Total"])
    ...
    if not rec.empty:
        ...
        rp["#+!%"] = ((_num_series(rp["#"]) + _num_series(rp["+"]) + _num_series(rp["!"])) / _num_series(rp["Total"]).replace(0,1) * 100).round(1); rp["=%"] = (_num_series(rp["="]) / _num_series(rp["Total"]).replace(0,1) * 100).round(1)
    ...
    if not blk.empty:
        ...
        bp["Eff%"] = _safe_eff(bp["#"], bp["="], bp["Total"])
    ...
    if not rival.empty:
        ...
        rot["Balance"] = _num_series(rot["Puntos"]) - _num_series(rot["="])
...
def pdf_bytes(data, modules=None):
    ...
    story.append(Paragraph("VolleyVision Hub V2.3 · creado por Marc Riverola Castellà", styles["Title"])); story.append(Spacer(1,12))
...
# ──────────────────────────────────────────────────────────────
# V2.3 — estabilidad, responsive y módulos tácticos mejorados
# ──────────────────────────────────────────────────────────────
st.markdown(f"""<style>
/* responsive/layout hardening */
.pro-section,.volley-panel,.insight-card,.kpi,.match-report-wrap,.pro-table-wrap{{box-sizing:border-box;}}
.volley-panel{{min-height:0!important;height:auto!important;overflow:visible!important;padding:1rem!important;margin-bottom:1rem!important;}}
.volley-panel svg{{display:block;margin:.25rem auto;max-height:360px!important;}}
.pro-table-wrap{{max-width:100%;overflow:auto!important;}}
.mp-card-grid{{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(260px,1fr))!important;gap:1rem!important;align-items:stretch!important;}}
.compact-metrics{{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(135px,1fr))!important;gap:.65rem!important;margin:.85rem 0 1rem!important;}}
.main .block-container{{max-width:1680px!important;}}
.stTabs [data-baseweb="tab-panel"]{{overflow:visible!important;}}
.match-plan-print{{background:{P['card']};border:1px solid {P['border']};border-radius:22px;padding:1.2rem;margin:1rem 0;}}
.match-plan-print h3{{margin-top:0;color:{P['accent1']};}}
.tactical-list li{{margin:.5rem 0;color:{P['text']};line-height:1.4;}}
.rec-mini{{border:1px solid {P['border']};border-radius:18px;padding:.85rem;background:{P['card']};height:100%;}}
.rec-mini h4{{margin:.1rem 0 .5rem;color:{P['accent1']};font-size:.95rem;}}
.rec-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:.35rem;}}
.rec-cell{{background:{P['bg2']};border:1px solid {P['border']};border-radius:12px;padding:.55rem;text-align:center;font-weight:900;min-height:58px;}}
.rec-cell .zone{{font-size:.72rem;color:{P['muted']};letter-spacing:.05em;}}
.rec-cell .val{{font-size:1.05rem;color:{P['text']};}}
@media(max-width:1100px){{
  .main .block-container{{padding-left:.8rem!important;padding-right:.8rem!important;}}
  .score-teams{{gap:.5rem!important;}}
  .result{{font-size:2rem!important;}}
  table.match-report{{min-width:980px!important;font-size:.68rem!important;}}
}}
@media(max-width:760px){{
  .hero h1{{font-size:1.65rem!important;}}
  .quick-grid{{grid-template-columns:1fr!important;}}
  .stTabs [data-baseweb="tab"]{{font-size:.75rem!important;padding:7px 10px!important;}}
  .tname{{min-width:120px!important;font-size:.9rem!important;}}
}}
</style>""", unsafe_allow_html=True)


def _safe_num_col(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    out = df.copy()
    for c in cols:
        if c in out.columns:
...
def _serve_type_from_code(row) -> str:
    """Clasificación de saque según código DataVolley usado por Marc.
    Q = salto potencia, T = salto flotante, N/n = saque desde suelo.
    """
    code = str(row.get("tipo_code", "")).strip().upper()
    raw = str(row.get("raw", ""))
    if code == "Q":
        return "Q · salto potencia"
    if code == "T":
        return "T · salto flotante"
    if code == "N":
        return "N · saque suelo"
    # Fallbacks por si algún archivo trae texto normalizado
    t = str(row.get("tipo", "")).lower()
    if "pot" in t or "jump" in t:
        return "Q · salto potencia"
    if "float" in t or "flot" in t or "tenso" in t:
        return "T · salto flotante"
    if "suelo" in t or "standing" in t:
        return "N · saque suelo"
    return "Sin tipo"


def _plays_with_context(data: dict) -> pd.DataFrame:
    """Contexto táctico estable y rápido.
    K1 = equipo que recibe el saque. K2 = equipo que saca/defiende/contraataca.
    """
    plays = data.get("plays", pd.DataFrame()).copy()
    if plays.empty:
        return plays
    plays["_order"] = range(len(plays))
    defaults = {
        "fase":"Total", "fase_detalle":"", "recepcion_eval":"Sin recepción",
        "recepcion_zona":"Sin zona", "colocador":"Sin colocador", "rotacion":"Todas",
        "saque_origen":"Sin zona de saque", "saque_tipo":"Sin tipo"
    }
    for c,v in defaults.items():
        plays[c] = v
    def _z(v):
        sv = str(v).strip()
        return f"Z{sv}" if sv and sv not in ("nan","None") else "Sin zona"
    plays["origen"] = plays.apply(lambda r: _z(r.get("zona_fin")) if r.get("skill_code")=="R" else _z(r.get("zona_inicio")), axis=1)
    plays["destino"] = plays.apply(lambda r: _z(r.get("zona_fin")), axis=1)
    for (setn, rally), rally_grp in plays.groupby(["set","rally"], sort=False):
        rally_grp = rally_grp.sort_values("_order")
        serve_context = {}
        serves = rally_grp[rally_grp["skill_code"]=="S"]
        for _, sv in serves.iterrows():
            receiver = "away" if sv.get("equipo") == "home" else "home"
            serve_context[receiver] = {"saque_origen": _z(sv.get("zona_inicio")), "saque_tipo": _serve_type_from_code(sv)}
        for equipo, grp in rally_grp.groupby("equipo", sort=False):
            grp = grp.sort_values("_order")
            has_rec = (grp["skill_code"]=="R").any()
            has_srv = (grp["skill_code"]=="S").any()
            fase = "K1" if has_rec else "K2"
            detalle = "K1" if has_rec else ("K2 · saque" if has_srv else "K2 · contraataque")
            recs = grp[grp["skill_code"]=="R"]
            if not recs.empty:
                rec_eval = recs.iloc[0].get("eval_code") or "Sin recepción"
                rec_zone = _z(recs.iloc[0].get("zona_fin"))
            else:
                rec_eval, rec_zone = "Sin recepción", "Sin zona"
            rv = grp.iloc[0].get("rotation", None)
            if pd.notna(rv) and str(rv).strip() not in ("", "None"):
                try: rot = f"P{int(float(rv))}"
                except Exception: rot = f"P{rv}"
            else:
                score_col = "home_score" if equipo=="home" else "away_score"
                base = pd.to_numeric(pd.Series([grp.iloc[0].get(score_col,0)]), errors="coerce").fillna(0).iloc[0]
                rot = f"P{(int(base)%6)+1}"
            sc = serve_context.get(equipo, {"saque_origen":"Sin zona de saque", "saque_tipo":"Sin tipo"})
            plays.loc[grp.index, ["fase","fase_detalle","recepcion_eval","recepcion_zona","rotacion","saque_origen","saque_tipo"]] = [fase, detalle, rec_eval, rec_zone, rot, sc["saque_origen"], sc["saque_tipo"]]
            # colocador: último E antes del ataque; fallback al jugador de E si existe en rally
            last_setter = "Sin colocador"
            for idx,row in grp.iterrows():
                if row.get("skill_code") == "E":
                    last_setter = row.get("jugador", "Sin colocador") or "Sin colocador"
...
def _reception_zone_summary(rec: pd.DataFrame, zone_col="origen") -> pd.DataFrame:
    if rec is None or rec.empty:
        return pd.DataFrame(columns=["Zona","Total","#","+","!","-","/","=","#%","#+!%","=%","Eff%"])
    rows=[]
    for z,g in rec.groupby(zone_col, dropna=False):
        c=_eval_counts(g); total=len(g); pos=c["#"]+c["+"]+c["!"]
        rows.append({"Zona": z if str(z).startswith("Z") else "Sin zona", "Total":total,
                     "#":c["#"], "+":c["+"], "!":c["!"], "-":c["-"], "/":c["/"], "=":c["="],
                     "#%":round(c["#"]/max(total,1)*100,1), "#+!%":round(pos/max(total,1)*100,1),
                     "=%":round(c["="]/max(total,1)*100,1), "Eff%":round((pos-c["="])/max(total,1)*100,1)})
    out=pd.DataFrame(rows)
    return _safe_num_col(out, ["Total","#","+","!","-","/","=","#%","#+!%","=%","Eff%"])


def _zone_cards_html(zone_df: pd.DataFrame, value="Eff%", title=""):
    order = [["Z4","Z3","Z2"],["Z7","Z8","Z9"],["Z5","Z6","Z1"]]
    vals = {}
    if zone_df is not None and not zone_df.empty and "Zona" in zone_df.columns:
        for _,r in zone_df.iterrows():
            vals[str(r.get("Zona"))] = r
    html = f"<div class='rec-mini'><h4>{title}</h4><div class='rec-grid'>"
    for row in order:
        for z in row:
            r = vals.get(z)
            v = float(r.get(value,0)) if r is not None and value in r else 0
            total = int(r.get("Total",0)) if r is not None and "Total" in r else 0
            bg = P['accent1'] if total else ("#332616" if DARK else "#fff4dd")
            color = "#111827" if total else P['muted']
            html += f"<div class='rec-cell' style='background:{bg};color:{color}'><div class='zone'>{z}</div><div class='val'>{v:.0f}%</div><div style='font-size:.68rem'>Nº {total}</div></div>"
    html += "</div></div>"
    return html


def _reception_type_bucket(rec: pd.DataFrame, bucket: str) -> pd.DataFrame:
    if rec.empty: return rec
    s = rec["saque_tipo"].astype(str)
    if bucket == "Q": return rec[s.str.startswith("Q")]
    if bucket == "T": return rec[s.str.startswith("T")]
    if bucket == "N": return rec[s.str.startswith("N")]
    return rec
...
def render_reception(data, key="rec"):
    ...
    with c1: receptor = st.selectbox("Receptor", ["Todos"] + sorted(rec["jugador"].dropna().unique()), key=f"{key}_player")
    with c2: tipo_saque = st.selectbox("Tipo de saque recibido", ["Todos","Q · salto potencia","T · salto flotante","N · saque suelo","Sin tipo"], key=f"{key}_type")
    rec = _filter_df(rec, "jugador", receptor)
    rec = _filter_df(rec, "saque_tipo", tipo_saque)
    if rec.empty:
        st.info("No hay recepciones con esta selección."); return
    cnt=_eval_counts(rec); total=len(rec); pos=cnt["#"]+cnt["+"]+cnt["!"]
    _metric_strip([("Recepciones", total),("#",cnt["#"]),("+",cnt["+"]),("!",cnt["!"]),("-",cnt["-"]),("/",cnt["/"]),("=",cnt["="]),("#%",f"{round(cnt['#']/max(total,1)*100,1)}%"),("#+!%",f"{round(pos/max(total,1)*100,1)}%"),("Eff%",f"{round((pos-cnt['='])/max(total,1)*100,1)}%")])

    st.markdown("### Recepción por tipo de saque")
    cols=st.columns(3)
    for col, code, label in zip(cols, ["Q","T","N"], ["Q · salto potencia", "T · salto flotante", "N · saque suelo"]):
        sub=_reception_type_bucket(rec, code); c=_eval_counts(sub); t=len(sub); p=c["#"]+c["+"]+c["!"]
        with col:
            st.markdown(f"<div class='insight-card'><div class='label'>{label}</div><div class='big'>{t}</div><div class='note'># {c['#']} · + {c['+']} · ! {c['!']} · - {c['-']} · / {c['/']} · = {c['=']}<br># {round(c['#']/max(t,1)*100,1)}% · #+! {round(p/max(t,1)*100,1)}% · Eff {round((p-c['='])/max(t,1)*100,1)}%</div></div>", unsafe_allow_html=True)

    zones=_reception_zone_summary(rec, "origen")
    left,right=st.columns([.95,1.35])
    with left:
        st.markdown(_zone_cards_html(zones, "Eff%", f"Mapa recepción · {team_label}"), unsafe_allow_html=True)
    with right:
        pro_table(zones.sort_values("Total", ascending=False), height=360, caption="Recepción por zona")

    st.markdown("### Recepción según zona de saque")
    def serve_group(z):
        z=str(z).replace("Z","").strip()
        if z in ["1","9"]: return "Saque desde Z1/Z9"
        if z == "6": return "Saque desde Z6"
        if z in ["7","5"]: return "Saque desde Z7/Z5"
        return "Sin zona"
    rec2=rec.copy(); rec2["Grupo saque"]=rec2["saque_origen"].apply(serve_group)
    cols=st.columns(3); full=[]
    for col,group in zip(cols,["Saque desde Z1/Z9","Saque desde Z6","Saque desde Z7/Z5"]):
        sub=rec2[rec2["Grupo saque"]==group]
        summ=_reception_zone_summary(sub,"origen")
        if not summ.empty:
            tmp=summ.copy(); tmp.insert(0,"Grupo saque",group); full.append(tmp)
        with col: st.markdown(_zone_cards_html(summ,"Eff%",group), unsafe_allow_html=True)
    if full:
        pro_table(pd.concat(full, ignore_index=True).sort_values(["Grupo saque","Total"], ascending=[True,False]), height=340, caption="Detalle recepción por zona de saque")
    st.markdown("### Zonas de contacto")
    st.info("Los .dvw cargados no incluyen de forma fiable derecha/izquierda/medio/arriba/abajo. El bloque queda preparado para activarse cuando esos códigos estén disponibles.")


def _base_direction_court(title: str, skill: str = "A"):
    fig = go.Figure()
    bg = "#f8fafc" if not DARK else "#0f172a"; court = "#f2a11a"; line = "#ffffff"; muted = "#334155" if not DARK else "#cbd5e1"
    fig.add_shape(type="rect", x0=.1,x1=3.9,y0=.15,y1=6.85,line=dict(color=P['border'],width=1),fillcolor=bg,layer="below")
    for y0,y1,name in [(0.65,3.25,"ORIGEN"),(3.75,6.35,"DESTINO")]:
        fig.add_shape(type="rect",x0=.55,x1=3.45,y0=y0,y1=y1,line=dict(color=line,width=3),fillcolor=court,opacity=.96,layer="below")
        fig.add_shape(type="line",x0=.55,x1=3.45,y0=(y0+y1)/2,y1=(y0+y1)/2,line=dict(color=line,width=1.2,dash="dot"),layer="below")
        for x in [1.52,2.48]: fig.add_shape(type="line",x0=x,x1=x,y0=y0,y1=y1,line=dict(color=line,width=1,dash="dot"),layer="below")
        fig.add_annotation(x=.25,y=(y0+y1)/2,text=name,showarrow=False,textangle=-90,font=dict(size=10,color=muted))
    fig.add_shape(type="line",x0=.45,x1=3.55,y0=3.5,y1=3.5,line=dict(color="#111827" if not DARK else "#f8fafc",width=6))
    fig.add_shape(type="line",x0=.45,x1=3.55,y0=3.5,y1=3.5,line=dict(color=line,width=1,dash="dot"))
    for z in ["Z4","Z3","Z2","Z7","Z8","Z9","Z5","Z6","Z1"]:
        xt,yt=_dv_zone_xy(z,"target",skill); xs,ys=_dv_zone_xy(z,"source","A")
        fig.add_annotation(x=xt,y=yt,text=z,showarrow=False,font=dict(size=11,color="#0f172a"),bgcolor="rgba(255,255,255,.78)",borderpad=2)
        fig.add_annotation(x=xs,y=ys,text=z,showarrow=False,font=dict(size=11,color="#0f172a"),bgcolor="rgba(255,255,255,.78)",borderpad=2)
    if skill=="S":
        for z in ["Z5","Z7","Z6","Z9","Z1"]:
            xs,ys=_dv_zone_xy(z,"source","S")
            fig.add_annotation(x=xs,y=ys,text=f"S{z[-1]}",showarrow=False,font=dict(size=10,color="#0f172a"),bgcolor="#fef3c7",borderpad=2)
    fig.update_xaxes(range=[0,4],visible=False); fig.update_yaxes(range=[0,7],visible=False,scaleanchor="x")
    fig.update_layout(title=dict(text=title,x=.02,font=dict(size=16,color=P['text'])),height=500,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",margin=dict(t=48,b=8,l=8,r=8))
    return fig
...
def render_distribution(data, key="dist"):
    df, team_label = filtered_context(data, key)
    if df.empty:
        st.info("Sin datos con estos filtros."); return
    c1,c2,c3=st.columns(3)
    setters=sorted([x for x in df["colocador"].dropna().unique() if x and x!="Sin colocador"])
    with c1: setter=st.selectbox("Colocador", ["Todos"]+setters, key=f"{key}_setter")
    evals=sorted([x for x in df["recepcion_eval"].dropna().unique() if x]) or ["Sin recepción"]
    zones=sorted([x for x in df["recepcion_zona"].dropna().unique() if x]) or ["Sin zona"]
    with c2: recq=st.multiselect("Recepciones acumulables", evals, default=evals, key=f"{key}_recq")
    with c3: recz=st.multiselect("Zonas recepción acumulables", zones, default=zones, key=f"{key}_recz")
    view=_filter_df(df,"colocador",setter)
    if recq: view=view[view["recepcion_eval"].isin(recq)]
    if recz: view=view[view["recepcion_zona"].isin(recz)]
    attacks=view[view["skill_code"]=="A"].copy()
    if attacks.empty:
        st.info("Sin ataques/colocaciones con estos filtros. Prueba fase Total o añade Sin recepción en K2."); return
    left,right=st.columns([1.25,1])
    with left: distribution_grid(attacks, f"Distribución · {team_label}")
    with right:
        attacks["zona_tactica"]=attacks["origen"].apply(_distribution_zone_group)
        zone_table=attacks.groupby("zona_tactica").agg(Balones=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        zone_table["Zona"]=zone_table["zona_tactica"].map(DISTRIBUTION_ZONE_LABELS).fillna(zone_table["zona_tactica"])
        zone_table["Distribución %"]=(zone_table["Balones"]/max(len(attacks),1)*100).round(1); zone_table["Eff%"]=_safe_eff(zone_table["#"], zone_table["="], zone_table["Balones"])
        pro_table(zone_table[["Zona","Balones","#","=","Distribución %","Eff%"]].sort_values("Balones",ascending=False), height=330, caption="Resumen por salida táctica")
    st.markdown("### Jugadas más usadas")
    detail=attacks.copy(); detail["Jugada"]=detail.get("combo","").fillna("").replace("","Sin jugada") if "combo" in detail.columns else "Sin jugada"
    detail["Salida"]=detail["origen"].apply(_distribution_zone_group).map(DISTRIBUTION_ZONE_LABELS).fillna(detail["origen"])
    combo=detail.groupby("Jugada").agg(Total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
    combo["Distribución %"]=(combo["Total"]/max(len(detail),1)*100).round(1); combo["Eff%"]=_safe_eff(combo["#"],combo["="],combo["Total"])
    ctx=[]
    for j in combo["Jugada"]:
        d=detail[detail["Jugada"]==j]
        ctx.append((d["rotacion"].mode().iloc[0] if not d["rotacion"].mode().empty else "-", d["recepcion_eval"].mode().iloc[0] if not d["recepcion_eval"].mode().empty else "-", d["Salida"].mode().iloc[0] if not d["Salida"].mode().empty else "-"))
    combo["Rotación más usada"]=[x[0] for x in ctx]; combo["Recepción habitual"]=[x[1] for x in ctx]; combo["Salida habitual"]=[x[2] for x in ctx]
    pro_table(combo.sort_values(["Total","Eff%"],ascending=[False,False]), height=420, caption="Combinaciones/jugadas más utilizadas")
...
def render_block(data, key="blk"):
    ...
    blk=df[df["skill_code"]=="B"].copy()
    total=len(blk); c=_eval_counts(blk); bal=c["#"]-c["="]
    _metric_strip([("Acciones BLQ",total),("BLQ #",c["#"]),("BLQ =",c["="]),("Balance",bal),("Eff%",f"{round(bal/max(total,1)*100,1)}%")])
    if blk.empty:
        st.info("No hay acciones de bloqueo codificadas con estos filtros."); return
    rank=blk.groupby(["jugador","dorsal","rotacion"]).agg(Acciones=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
    rank=_safe_num_col(rank,["Acciones","#","="]); rank["Balance"]=rank["#"]-rank["="]; rank["Eff%"]=_safe_eff(rank["#"],rank["="],rank["Acciones"])
    left,right=st.columns(2)
    with left: pro_table(rank.sort_values(["#","Balance"],ascending=False), height=360, caption="Ranking de bloqueadores")
    with right:
        rot=blk.groupby("rotacion").agg(Acciones=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        rot=_safe_num_col(rot,["Acciones","#","="]); rot["Balance"]=rot["#"]-rot["="]; rot["Eff%"]=_safe_eff(rot["#"],rot["="],rot["Acciones"])
        pro_table(rot.sort_values("Acciones",ascending=False), height=360, caption="Bloqueo por rotación")
    zone=blk[blk["origen"].astype(str).str.startswith("Z")].groupby("origen").agg(Acciones=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index().rename(columns={"origen":"Zona"})
    if not zone.empty:
        zone=_safe_num_col(zone,["Acciones","#","="]); zone["Eff%"]=_safe_eff(zone["#"],zone["="],zone["Acciones"])
        st.markdown(_zone_cards_html(zone.rename(columns={"Acciones":"Total"}), "Eff%", "Mapa de bloqueo por zona"), unsafe_allow_html=True)
        pro_table(zone.sort_values("Acciones",ascending=False), height=300, caption="Zonas de bloqueo")


def generate_match_plan(data, my_team, rival_team, fase="Total", desde=0):
    ctx=_plays_with_context(data)
    teams={data["home_team"]["name"]:"home", data["away_team"]["name"]:"away"}
    rival_code=teams.get(rival_team); my_code=teams.get(my_team)
    plan={"claves":[],"acciones":[],"tablas":{},"texto":[]}
    if rival_code is None or ctx.empty:
        plan["claves"].append(("Sin datos", "No hay información suficiente", "Carga partidos del rival.")); return plan
    rival=ctx[ctx["equipo"]==rival_code].copy(); my=ctx[ctx["equipo"]==my_code].copy() if my_code else pd.DataFrame()
    rival=_filter_df(rival,"fase",fase)
    if desde>0:
        score_col="home_score" if rival_code=="home" else "away_score"
        rival=rival[pd.to_numeric(rival[score_col],errors="coerce").fillna(0)>=desde]
    att=rival[rival["skill_code"]=="A"].copy(); rec=rival[rival["skill_code"]=="R"].copy(); blk=rival[rival["skill_code"]=="B"].copy()
    if not att.empty:
        az=att.groupby("origen").agg(Total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        az=_safe_num_col(az,["Total","#","="]); az["Volumen %"]=(az["Total"]/max(len(att),1)*100).round(1); az["Eff%"]=_safe_eff(az["#"],az["="],az["Total"])
        plan["tablas"]["Ataque rival por zona"]=az.sort_values("Total",ascending=False)
        top=az.sort_values("Total",ascending=False).iloc[0]; eff=az.sort_values("Eff%",ascending=False).iloc[0]
        plan["claves"].append(("Prioridad de bloqueo", f"El rival carga el ataque por {top['origen']}", f"{int(top['Total'])} balones · {top['Volumen %']}% del volumen."))
        plan["claves"].append(("Zona más peligrosa", f"La zona más eficiente del rival es {eff['origen']}", f"Eff {eff['Eff%']}%."))
        ap=att.groupby("jugador").agg(Total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        ap=_safe_num_col(ap,["Total","#","="]); ap["Eff%"]=_safe_eff(ap["#"],ap["="],ap["Total"]); ap["Volumen %"]=(ap["Total"]/max(len(att),1)*100).round(1)
        plan["tablas"]["Atacantes rivales"]=ap.sort_values(["Total","Eff%"],ascending=False)
        p=ap.sort_values(["Total","Eff%"],ascending=False).iloc[0]
        plan["acciones"].append(("Bloqueo", f"Preparar plan específico sobre {p['jugador']}", f"{int(p['Total'])} ataques · Eff {p['Eff%']}%."))
    if not rec.empty:
        rz=_reception_zone_summary(rec,"origen").sort_values(["=%","Total"],ascending=[False,False])
        plan["tablas"]["Recepción rival por zona"]=rz
        if not rz.empty:
            z=rz.iloc[0]; plan["acciones"].append(("Saque", f"Dirigir saque a {z['Zona']}", f"Error {z['=%']}% · #+! {z['#+!%']}%."))
        rp=rec.groupby("jugador").agg(Total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "#+!":("eval_code",lambda x:x.isin(["#","+","!"]).sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        rp=_safe_num_col(rp,["Total","#","#+!","="]); rp["#%"]=(rp["#"]/rp["Total"].replace(0,1)*100).round(1); rp["#+!%"]=(rp["#+!"]/rp["Total"].replace(0,1)*100).round(1); rp["=%"]=(rp["="]/rp["Total"].replace(0,1)*100).round(1)
        plan["tablas"]["Receptores rivales"]=rp.sort_values(["=%","Total"],ascending=[False,False])
        if not rp.empty:
            r=rp.sort_values(["=%","Total"],ascending=[False,False]).iloc[0]; plan["claves"].append(("Receptor a presionar", f"Buscar a {r['jugador']}", f"{int(r['Total'])} recepciones · = {r['=%']}%."))
    if not blk.empty:
        bp=blk.groupby("jugador").agg(Acciones=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()), "=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
        bp=_safe_num_col(bp,["Acciones","#","="]); bp["Balance"]=bp["#"]-bp["="]
        plan["tablas"]["Bloqueo rival"]=bp.sort_values("#",ascending=False)
    if not rival.empty:
        rot=rival.groupby("rotacion").agg(Acciones=("skill_code","count"), Puntos=("es_punto","sum"), **{"=":("es_error","sum")}).reset_index()
        rot=_safe_num_col(rot,["Acciones","Puntos","="]); rot["Balance"]=rot["Puntos"]-rot["="]
        plan["tablas"]["Rotaciones rivales"]=rot.sort_values(["Balance","Acciones"],ascending=[True,False])
        weak=rot.sort_values(["Balance","Acciones"],ascending=[True,False]).iloc[0]
        plan["acciones"].append(("Rotación", f"Presionar cuando el rival esté en {weak['rotacion']}", f"Balance {int(weak['Balance'])}."))
    # narrative coaching notes
    plan["texto"]=[
        "Objetivo del plan: reducir las situaciones cómodas del colocador rival y orientar nuestro saque para facilitar bloqueo-defensa.",
        "Prioridad 1: sacar al receptor/zona con mayor porcentaje de error o menor #+!%, especialmente desde punto 18.",
        "Prioridad 2: cerrar primero la zona de mayor volumen de ataque y ajustar defensa sobre la dirección más repetida.",
        "Prioridad 3: detectar la rotación rival vulnerable y aumentar presión de saque en ese momento."
    ]
    if not plan["claves"]: plan["claves"].append(("Muestra insuficiente","Carga más partidos del rival","El match plan mejora con acumulado."))
    return plan
...
def render_match_plan(data, key="mp"):
    ...
    with c4: desde=st.selectbox("Desde punto", list(range(0,31)), key=f"{key}_desde")
    if my_team==rival_team:
        st.warning("Elige equipos diferentes."); return
    plan=generate_match_plan(data,my_team,rival_team,fase,desde)
    st.markdown(f"<div class='mp-header'><h1>Match Plan · {my_team} vs {rival_team}</h1><p>Informe táctico para banquillo/jugadores · Fase {fase} · Desde punto {desde}</p></div>", unsafe_allow_html=True)
    st.markdown("### Resumen ejecutivo")
    st.markdown("<div class='mp-card-grid'>", unsafe_allow_html=True)
    for title,action,evidence in plan.get("claves",[])[:6]:
        st.markdown(f"<div class='insight-card'><div class='label'>{title}</div><div style='font-size:1.02rem;font-weight:950;color:{P['text']};margin:.35rem 0'>{action}</div><div class='note'>{evidence}</div></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    tabs=st.tabs(["Hoja rápida", "Plan de saque", "Bloqueo-defensa", "Ataque rival", "Recepción rival", "Rotaciones"])
    with tabs[0]:
        st.markdown("<div class='match-plan-print'><h3>Hoja rápida imprimible</h3><ul class='tactical-list'>", unsafe_allow_html=True)
        for t in plan.get("texto",[]): st.markdown(f"<li>{t}</li>", unsafe_allow_html=True)
        for title,action,evidence in plan.get("acciones",[])[:8]: st.markdown(f"<li><b>{title}:</b> {action}<br><span style='color:{P['muted']}'>{evidence}</span></li>", unsafe_allow_html=True)
        st.markdown("</ul></div>", unsafe_allow_html=True)
    with tabs[1]:
        if "Recepción rival por zona" in plan["tablas"]: pro_table(plan["tablas"]["Recepción rival por zona"], height=330, caption="Dónde sacar")
        if "Receptores rivales" in plan["tablas"]: pro_table(plan["tablas"]["Receptores rivales"], height=330, caption="A quién sacar")
    with tabs[2]:
        if "Ataque rival por zona" in plan["tablas"]: pro_table(plan["tablas"]["Ataque rival por zona"], height=330, caption="Prioridad de bloqueo por zona")
        if "Bloqueo rival" in plan["tablas"]: pro_table(plan["tablas"]["Bloqueo rival"], height=260, caption="Bloqueadores rivales")
    with tabs[3]:
        if "Atacantes rivales" in plan["tablas"]: pro_table(plan["tablas"]["Atacantes rivales"], height=380, caption="Atacantes rivales")
        if "Ataque rival por zona" in plan["tablas"]: pro_table(plan["tablas"]["Ataque rival por zona"], height=280, caption="Ataque por zona")
    with tabs[4]:
        if "Recepción rival por zona" in plan["tablas"]: pro_table(plan["tablas"]["Recepción rival por zona"], height=330, caption="Recepción por zona")
        if "Receptores rivales" in plan["tablas"]: pro_table(plan["tablas"]["Receptores rivales"], height=330, caption="Receptores")
    with tabs[5]:
        if "Rotaciones rivales" in plan["tablas"]: pro_table(plan["tablas"]["Rotaciones rivales"], height=340, caption="Rotaciones rivales")
...
def main():
    ...
    st.markdown('<footer>VolleyVision Hub V2.3 · creado por Marc Riverola Castellà</footer>', unsafe_allow_html=True)
