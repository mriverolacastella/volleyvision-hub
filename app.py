...
.match-report-title{font-size:1.55rem;font-weight:950;color:#f59e0b;margin:1.6rem 0 .8rem}
.match-report-wrap{border:1px solid #2b3445;border-radius:18px;overflow:auto;margin:.6rem 0 1.8rem;background:#0d1117;box-shadow:0 16px 38px rgba(0,0,0,.22)}
table.match-report{width:100%;border-collapse:collapse;font-size:.78rem;color:#f8fafc;min-width:1280px}
table.match-report th{background:#111827;color:#cbd5e1;text-transform:uppercase;font-weight:950;letter-spacing:.04em;padding:.65rem .55rem;border:1px solid #273044;text-align:center;position:sticky;top:0;z-index:2}
table.match-report td{padding:.58rem .55rem;border:1px solid #273044;text-align:center;font-weight:750;background:#16181f}
table.match-report td.name{text-align:left;font-weight:950;max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;background:#181b24}
table.match-report td.good{background:#11451f;color:#fff}
table.match-report td.bad{background:#8f1d1d;color:#fff}
table.match-report tr.total td{background:#2a2b37!important;font-weight:950}
...
def _plays_with_context(data: dict) -> pd.DataFrame:
    ...
    for (setn, rally), rally_grp in plays.groupby(["set", "rally"], sort=False):
        for equipo, grp in rally_grp.groupby("equipo", sort=False):
            ...
            plays.loc[grp.index, ["fase","fase_detalle","recepcion_eval","recepcion_zona","rotacion"]] = [fase, detalle, rec_eval, rec_zone, rot]
...
def filtered_context(data, key, team_default=None):
    ...
    st.markdown("<div class='phase-help'><b>K1</b>: acciones cuando el equipo recibe el saque. <b>K2</b>: acciones cuando el equipo saca, defiende o contraataca.</div>", unsafe_allow_html=True)
...
def render_reception(data, key="rec"):
    ...
    if rec.empty: st.info("Sin recepciones con estos filtros."); return
    c1,c2=st.columns(2)
    with c1: receptor=st.selectbox("Receptor", ["Todos"]+sorted(rec["jugador"].dropna().unique()), key=f"{key}_player")
    with c2: tipo_saque=st.selectbox("Tipo de saque recibido", ["Todos"]+sorted([x for x in rec["tipo"].dropna().unique() if x]), key=f"{key}_type")
    rec=_filter_df(_filter_df(rec,"jugador",receptor),"tipo",tipo_saque)
    c1,c2,c3,c4,c5=st.columns(5)
    with c1: kpi("Recepciones", int(len(rec)))
    with c2: kpi("#", int((rec["eval_code"]=="#").sum()))
    with c3: kpi("#+!%", f"{round(rec['eval_code'].isin(['#','+','!']).sum()/max(len(rec),1)*100,1)}%")
    with c4: kpi("=", int((rec["eval_code"]=="=").sum()))
    with c5: kpi("Eff%", f"{round((rec['eval_code'].isin(['#','+','!']).sum()-(rec['eval_code']=='=').sum())/max(len(rec),1)*100,1)}%")
    zones=rec[rec["origen"]!="Sin zona"].groupby("origen").agg(total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()),"+":("eval_code",lambda x:(x=="+").sum()),"!":("eval_code",lambda x:(x=="!").sum()),"-":("eval_code",lambda x:(x=="-").sum()),"/":("eval_code",lambda x:(x=="/").sum()),"=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
    if not zones.empty:
        zones["#+!%"] = ((zones["#"]+zones["+"]+zones["!"])/zones["total"].replace(0,1)*100).round(1); zones["#%"]=(zones["#"]/zones["total"].replace(0,1)*100).round(1); zones["Eff%"] = (((zones["#"]+zones["+"]+zones["!"])-zones["="])/zones["total"].replace(0,1)*100).round(1)
    left,right=st.columns([.9,1.1])
    with left: st.markdown(court_svg(dict(zip(zones["origen"], zones["Eff%"].fillna(0).astype(int))) if not zones.empty else {}, f"Eff% recepción · {team_label}", value_suffix="%"), unsafe_allow_html=True)
    with right: pro_table(zones.sort_values("total",ascending=False) if not zones.empty else zones, height=360, caption='Recepción por zona')
    ...
    rec2['Grupo saque']=rec2['destino'].apply(serve_group)
    by=rec2.groupby(['Grupo saque','origen']).agg(total=("skill_code","count"), **{"#":("eval_code",lambda x:(x=="#").sum()),"+":("eval_code",lambda x:(x=="+").sum()),"!":("eval_code",lambda x:(x=="!").sum()),"-":("eval_code",lambda x:(x=="-").sum()),"/":("eval_code",lambda x:(x=="/").sum()),"=":("eval_code",lambda x:(x=="=").sum())}).reset_index()
    if not by.empty: by['#+!%']=((by['#']+by['+']+by['!'])/by['total'].replace(0,1)*100).round(1); by['Eff%']=(((by['#']+by['+']+by['!'])-by['='])/by['total'].replace(0,1)*100).round(1)
    pro_table(by.sort_values(['Grupo saque','total'], ascending=[True,False]), height=360, caption='Zonas de recepción según zona de saque')
    st.markdown("#### Zonas de contacto")
    st.caption("Bloque preparado para derecha/izquierda/medio/arriba/abajo cuando el DVW lo incluya de forma codificada.")
    pro_table(pd.DataFrame({"Zona contacto":["Derecha","Izquierda","Medio","Arriba","Abajo"],"Total":[0,0,0,0,0],"#+!%":["No disponible"]*5,"=":[0,0,0,0,0]}), height=220, caption='Zonas de contacto en recepción')
...
def render_user_manual():
    ...
    <div class='manual-card'>
      <h3>1. Sube uno o varios archivos .dvw</h3>
      <p>VolleyVision Hub analiza archivos de DataVolley. Primero carga el partido o varios partidos y pulsa <b>Analizar partidos</b>.</p>
    </div>
    <div class='manual-card'>
      <h3>2. Explora las pestañas</h3>
      <p><b>General</b> resume el partido y el Match Report. <b>Ataque</b>, <b>Saque</b>, <b>Recepción</b> y <b>Distribución</b> permiten filtrar por equipo, rotación, fase y punto. <b>Match Plan</b> genera un informe imprimible contra el rival.</p>
    </div>
    <div class='manual-card'>
      <h3>3. Símbolos DataVolley</h3>
      <p><b>#</b> punto/perfecta · <b>+</b> positiva · <b>!</b> media · <b>-</b> negativa · <b>/</b> bloqueada/continuidad negativa · <b>=</b> error.</p>
    </div>
...
def generate_match_plan(data: dict, my_team: str, rival_team: str, fase: str = "Total", desde: int = 0) -> dict:
    ...
    rival = _apply_matchplan_filters(ctx, rival_code, fase, desde); mine = _apply_matchplan_filters(ctx, my_code, fase, desde)
    att = rival[rival["skill_code"] == "A"].copy(); rec = rival[rival["skill_code"] == "R"].copy(); blk = rival[rival["skill_code"] == "B"].copy()
    plan = {"mi_equipo": my_team, "rival": rival_team, "fase": fase, "desde": desde, "claves": [], "tablas": {}}
    if not att.empty:
        ...
        az['Volumen %']=(az['Total']/max(len(att),1)*100).round(1); az['Eff%']=((az['#']-az['='])/az['Total'].replace(0,1)*100).round(1); plan['tablas']['ataque_rival']=az.sort_values('Total', ascending=False)
        top=az.sort_values('Total',ascending=False).iloc[0]; best=az.sort_values(['Eff%','Total'],ascending=False).iloc[0]
        plan['claves'].append(('Prioridad de bloqueo', f"El rival carga más el ataque por {top['origen']}.", f"{int(top['Total'])} balones · {top['Volumen %']}% del volumen"))
        plan['claves'].append(('Zona más peligrosa', f"La zona más eficiente del rival es {best['origen']}.", f"Eff {best['Eff%']}%"))
        byp=att.groupby('jugador').agg(Total=('skill_code','count'), **{"#":('eval_code',lambda x:(x=='#').sum()), "=":('eval_code',lambda x:(x=='=').sum()), "/":('eval_code',lambda x:x.isin(['/','-']).sum())}).reset_index(); byp['Eff%']=((byp['#']-byp['='])/byp['Total'].replace(0,1)*100).round(1); plan['tablas']['atacantes_rival']=byp.sort_values('Total', ascending=False)
    if not rec.empty:
        rz=rec[rec['origen']!='Sin zona'].groupby('origen').agg(Total=('skill_code','count'), **{"#":('eval_code',lambda x:(x=='#').sum()), "+":('eval_code',lambda x:(x=='+').sum()), "!":('eval_code',lambda x:(x=='!').sum()), "=":('eval_code',lambda x:(x=='=').sum())}).reset_index(); rz['#+!%']=((rz['#']+rz['+']+rz['!'])/rz['Total'].replace(0,1)*100).round(1); rz['=%']=(rz['=']/rz['Total'].replace(0,1)*100).round(1); plan['tablas']['recepcion_rival']=rz.sort_values(['=%','Total'], ascending=[False,False])
        worst=rz.sort_values(['=%','Total'], ascending=[False,False]).iloc[0]; plan['claves'].append(('Plan de saque', f"Sacar hacia {worst['origen']} para castigar la recepción rival.", f"= {worst['=%']}% · #+! {worst['#+!%']}%"))
        rp=rec.groupby('jugador').agg(Total=('skill_code','count'), **{"#":('eval_code',lambda x:(x=='#').sum()), "=":('eval_code',lambda x:(x=='=').sum())}).reset_index(); rp['=%']=(rp['=']/rp['Total'].replace(0,1)*100).round(1); plan['tablas']['receptores_rival']=rp.sort_values(['=%','Total'], ascending=[False,False])
    if not blk.empty:
        bp=blk.groupby('jugador').agg(Total=('skill_code','count'), **{"#":('eval_code',lambda x:(x=='#').sum()), "=":('eval_code',lambda x:(x=='=').sum())}).reset_index(); plan['tablas']['bloqueo_rival']=bp.sort_values('#', ascending=False)
        if not bp.empty: b=bp.sort_values('#', ascending=False).iloc[0]; plan['claves'].append(('Bloqueo rival', f"Vigilar al bloqueador {b['jugador']}.", f"{int(b['#'])} bloqueos punto detectados"))
    if not rival.empty:
        rot=rival.groupby('rotacion').agg(Acciones=('skill_code','count'), Puntos=('es_punto','sum'), **{"=":('es_error','sum')}).reset_index(); rot['Balance']=rot['Puntos']-rot['=']; plan['tablas']['rotaciones_rival']=rot.sort_values(['Balance','Acciones'], ascending=[True,False])
        weak=rot.sort_values(['Balance','Acciones'], ascending=[True,False]).iloc[0]; plan['claves'].append(('Rotación a presionar', f"La rotación rival más atacable es {weak['rotacion']}.", f"Balance {int(weak['Balance'])}"))
    if not plan['claves']: plan['claves'].append(('Muestra insuficiente','No hay suficientes datos con estos filtros.','Carga más partidos o usa fase Total.'))
    return plan

def render_match_plan(data, key="mp"):
    st.markdown("### Match Plan imprimible")
    st.caption("Selecciona tu equipo y el rival. El informe queda preparado para imprimir y usar durante el partido.")
    ...
    if my_team==rival_team: st.warning("Elige equipos diferentes para generar un match plan real."); return
    plan=generate_match_plan(data,my_team,rival_team,fase,desde)
    st.markdown(f"## Match Plan · {my_team} vs {rival_team}")
    st.markdown("#### Resumen ejecutivo")
    cols=st.columns(2)
    for i,(title,action,evidence) in enumerate(plan['claves'][:8]):
        with cols[i%2]: st.markdown(f"<div class='insight-card'><div class='label'>{title}</div><div style='font-size:1.02rem;font-weight:950;color:{P['text']};margin:.35rem 0'>{action}</div><div class='note'>{evidence}</div></div>", unsafe_allow_html=True)
    st.markdown("---")
    tabs=st.tabs(["Plan de saque", "Bloqueo-defensa", "Ataque rival", "Recepción rival", "Rotaciones", "Informe para imprimir"])
    with tabs[0]:
    ...
    with tabs[1]:
        if 'ataque_rival' in plan['tablas']: pro_table(plan['tablas']['ataque_rival'], height=320, caption='Prioridades de bloqueo por zona de ataque')
        if 'bloqueo_rival' in plan['tablas']: pro_table(plan['tablas']['bloqueo_rival'], height=260, caption='Bloqueadores rivales a vigilar')
        st.info("Criterio táctico: recepción rival #/+ = atención al central. Recepción rival -/=/! = cerrar salida más repetida.")
    with tabs[2]:
        if 'atacantes_rival' in plan['tablas']: pro_table(plan['tablas']['atacantes_rival'], height=360, caption='Atacantes rivales por volumen')
    ...
    with tabs[4]:
        if 'rotaciones_rival' in plan['tablas']: pro_table(plan['tablas']['rotaciones_rival'], height=360, caption='Rotaciones rivales: balance bajo arriba')
    with tabs[5]:
        st.markdown("### Hoja rápida para banquillo")
        for title,action,evidence in plan['claves'][:8]: st.markdown(f"- **{title}:** {action} _({evidence})_")
        st.caption("Este bloque está pensado para exportar/imprimir y entregar a jugadores o cuerpo técnico.")
...
def view_landing():
    ...
    st.markdown("## Cargar archivos .dvw")
    st.markdown("<div class='manual-card'><h3>Empieza aquí</h3><p>Sube un partido o varios partidos en formato <b>.dvw</b>. Después se activarán todas las pestañas de análisis.</p></div>", unsafe_allow_html=True)
...
def render_single_match(data, prefix="single"):
    score_bar(data)
    tabs = st.tabs(["Inicio / Manual", "General", "Ataque", "Saque", "Recepción", "Colocador / Distribución", "Match Plan", "Por Set", "Exportar"])
    ...
    with tabs[8]:
        ...
        default_modules = ["Resumen", "Ataque", "Saque", "Recepción", "Distribución", "Match Plan"]
        export_modules = st.multiselect(
            "Módulos a exportar",
            ["Resumen", "Ataque", "Saque", "Recepción", "Distribución", "Match Plan", "Por Set", "Acciones crudas"],
