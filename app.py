...
def render_distribution(data, key="dist"):
    ...
    if not attacks.empty:
        ...
        table = detail.groupby(["fase_detalle","Zona táctica","origen","jugador","recepcion_eval","recepcion_zona"]).agg(Balones=("skill_code","count"), Kills=("es_punto","sum"), Errores=("es_error","sum")).reset_index()
        table["Eff%"] = ((table["Kills"]-table["Errores"])/table["Balones"].replace(0,1)*100).round(1)
        table = table.rename(columns={"origen":"Zona DV"})
...
def render_user_manual():
    st.markdown("### Manual rápido de uso")
    st.markdown("""
    ...
    <div class='manual-card'>
      ...
      <p><b>K1</b> significa que tu equipo recibe el saque y construye recepción → colocación → ataque. <b>K2</b> significa que tu equipo está en saque, bloqueo, defensa o contraataque. Los filtros de rotación, fase y desde punto se aplican a ataque, saque, recepción y distribución.</p>
    </div>
    <div class='manual-card'>
      <h3>3. Distribución</h3>
      <p>La pista de distribución se agrupa en 6 salidas tácticas: Z4, Z3, Z2, Z5+Z7, PIPE/Z6+Z8 y Z1+Z9. Así puedes ver rápido hacia dónde juega el colocador, sin perder las zonas DV originales en la tabla de detalle.</p>
    </div>
    <div class='manual-card'>
      <h3>4. Lectura de símbolos</h3>
      <p>Las tablas usan símbolos DataVolley: <b>#</b> punto/perfecta, <b>+</b> positiva, <b>!</b> media, <b>-</b> negativa, <b>/</b> bloqueada o continuidad negativa según fundamento, <b>=</b> error.</p>
    </div>
